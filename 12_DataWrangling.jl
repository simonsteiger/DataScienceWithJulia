...
Our company is constantly reaching out to customers (active and potential ones) with diverse marketing campaigns. We want to use advanced analytics to prepare a more targeted approach for the next campaign. As we keep partial sociodemographic information in the client base, we can utilize it for the purpose of the project. The `adult` dataset contains features describing the population, such as marital status, education level, or occupation, and, most importantly, the `target` feature indicates whether a person has an annual income higher than $50,000 USD. In the future—in a later project in this series—we will build a classification model on the dataset and apply it to our database to indicate which marketing content is more suitable for a given individual.

We want to understand the data and conduct necessary data cleaning steps before training various models. Exploratory data analysis will allow us to spot any issues with the data that may require additional actions and will familiarize us with each particular feature and the relationships between them. In the analysis, we most often want to emphasize the relationship between each independent variable and the target variable. Before starting the modeling, we want to understand how features collected in these datasets correlate with the target.

!!! danger "Disclaimer"
	Some sociodemographic features (such as gender, ethnicity, and race) may be prohibited by law for use in machine learning (ML) models in particular realms, such as credit scoring, admissions systems, or targeted marketing. In addition to these legal obligations, industry and public institutions are heavily promoting ethical and unbiased approaches in ML modeling. We use income as a target feature in this milestone as it’s a natural label for the Adult dataset, and we do not intend to promote a biased approach in any way. If you want to learn more about bias and ethical issues in artificial intelligence, you can review the resources on [Responsible AI](https://www.microsoft.com/en-us/ai/responsible-ai). We also recommend the following liveProjects focused on mitigating bias: [Mitigate Machine Learning Bias: A Health Data Project](https://www.manning.com/liveproject/mitigate-machine-learning-bias-a-health-data-project) and [Mitigate Machine Learning Bias in Mortgage Lending Data](https://www.manning.com/liveprojectseries/mitigate-machine-learning-bias-in-mortgage-lending-data-ser). If you prefer a book instead, check out [Weapons of Math Destruction](https://en.wikipedia.org/wiki/Weapons_of_Math_Destruction) by Cathy O’Neil. It contains many real-world examples of algorithms making possibly biased decisions that deeply affect our lives.
"""
...
adult_ref = CSV.read(FILENAME, header=COLNAMES, DataFrame, delim=',', ignorerepeated=true);
